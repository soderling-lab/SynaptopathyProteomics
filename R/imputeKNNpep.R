imputeKNNpep <- function(tp,groupBy=NULL, samples_to_ignore = NULL,
	...
	for (i in 1:length(tp_list)) {
		...
		# Create an expression that will be evaluated to dynamically 
		# cast the tidy data into a matrix (formula: y ~ x).
		tp_input <- as.data.table(tp_list[[i]])
		y <- paste(c(groupBy,"Accession","Sequence","Modifications"),
			  collapse=" + ")
		x <- paste(c("Sample"),
			  collapse=" + ")
		opts <- c(value.var="'Intensity'")
