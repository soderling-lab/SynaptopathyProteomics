...
TAMPOR <- function(dat,
  ...
  rownames(traits) <- traits$Batch.Channel ## added this line!
  ...
  ## One can remove GIS here, depending on whether these are (not) biologically
  # meaningful for comparisons downstream
  ...
  rownames(traits) <- traits$Batch.Channel # added line here!
  ...
  if (length(removeColumns.cleanDat) > 0) {
    ...
    # FIXME: Whats going on here?
    # print(relAbundanceNorm2) # not returned -- this was not executed
    relAbundanceNorm2 <- relAbundanceNorm2[, -match(removeColumns.cleanDat, colnames(relAbundanceNorm2))]
  }

  rownames(traits) <- traits$Batch.Channel # added this line!

  # failed here bc rownames(traits) was NULL
