...
#' description: Performs WGCNA given a nomralized expression data matrix and 
#' parameters (optional).
...
if (length(commandArgs(trailingOnly=TRUE)) == 0){
 ...
 if (as.character(Sys.info()[1]) == "Linux"){
  ...
  project_dir <- "D:/Projects/Synaptopathy-Proteomics"
  message("Working on Windows OS!")
  setwd(project_dir)
 }
 data_file  <- file.path(project_dir, "data","wtDat.Rds")
 params_file <- file.path(project_dir, "data","default_parameters.txt")
...
if (!is.na(args$parameters)) {
	...
	temp_params <- read.delim(args$parameters, header = FALSE, col.names = c("Parameter","Value"))
...
script_dir <- paste(root,"bin","radalib", sep="/")
...
#------------------------------------------------------------------------------
# ## Calculate simple kME ratio (in/out) as quality metric.
#------------------------------------------------------------------------------
...
modules <- split(net$colors, net$colors)

# Loop to calculate kMEin and kME out.
kMEratio <- matrix(NA,nrow=length(modules),ncol = 2)
colnames(kMEratio) <- c("kMEin","kMEout")
rownames(kMEratio) <- names(modules)
for (i in seq_along(names(modules))){
  color <- names(modules)[i]
  v <- names(modules[[color]])
  subkME <- (1-subset(kME,rownames(kME) %in% v))^2
  idx <- match(color,substring(colnames(subkME),4))
  kMEin <- sum(subkME[,idx])
  kMEout <- sum(subkME[,c(1:ncol(subkME))[-idx]])
  kMEratio[i,] <- cbind(kMEin,kMEout)
}

kMEratio <- as.data.frame(kMEratio)
kMEratio$ratio <- kMEratio$kMEin/kMEratio$kMEout # kmein should be small, kmeout should be big (distance space)

varRatios <- kMEratio$ratio
names(varRatios) <- rownames(kMEratio)

# sum variance ratios should be minimized!
# sum variance ratio in gray should be maximized! Negative is okay!
score <- sum(varRatios) - 2*varRatios['grey'] # Minimize this quantity!
