...
def graph_from_adjm(adjm, weighted=True, signed=True):
    ...
    if weighted:
        g = Graph.TupleList(edge_tuples, weights=True)
    if not weighted:
        g = Graph.TupleList(edge_tuples, weights=False)
    return g
...
def clusterMCL(
    ...
    cmd = [
        ...
        "--abc",  # input file.
        "-I",
        str(inflation),  # inflation parameter.
        "-te",
        str(ncores),  # number of cores.
        "-o",
        "-",
    ]  # output file.
    if quiet:
    ...
    partition = dict(
        zip(sum(modules, []), np.repeat(module_names, module_size, axis=0))
    )
    ...
    PartitionType = getattr(
        import_module("leidenalg"), quality_measures.get(quality_measure)
    )
