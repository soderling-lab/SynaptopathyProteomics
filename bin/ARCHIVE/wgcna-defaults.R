...
# Input data
#default_params$datExpr <- datExpr
...
default_params$maxBlockSize          <- 5000
default_params$blockSizePenaltyPower <- 5
default_params$nPreclusteringCenters <- as.integer(min(ncol(datExpr)/20, 100*ncol(datExpr)/default_params$maxBlockSize))
...
default_params$minModuleSize   <- min(20, ncol(datExpr)/2)
...
default_params$minCoreKMESize    <- (min(20, ncol(datExpr)/2))/3
