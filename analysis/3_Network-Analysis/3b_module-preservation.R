...
data_files <- list(
  ...
  module = list(
    Cortex = "2020-02-10_Cortex_Surprise_Module_Self_Preservation.RData",
...
if (replace_negative == "abs") {
  ...
  network_list <- lapply(net, function(x) {
    x[x < 0] <- 0
    return(x)
  })
}
if (self_preservation) {
  names(network_list) <- self
}
...
message(paste(
  "Evaluating preservation of", perm_test$discovery,
