...
# Set ggplots theme; utilize arial font.
...
nPep <- raw_peptide %>%
  group_by(Accession) %>%
  summarize(nPeptides = length(Sequence), .groups="drop")
...
tmp <- impute_protein
tmp$Peptides <- NULL
dm <- tmp %>% as.data.table() %>% as.matrix(rownames="Accession")

data(cortex_partition)
#data(striatum_partition)
partition <- cortex_partition

samples <- cortex_data$Meta
samples$Sample <- samples$ColumnName
samples$Model <- gsub(" ","",samples$Model)

prot_dt <- dm %>% as.data.table(keep.rownames="Accession") %>%
	reshape2::melt(id.var="Accession", value.var="Intensity")
colnames(prot_dt) <- c("Accession","Sample","Intensity")
prot_dt <- prot_dt %>% filter(!grepl("QC",Sample))
prot_dt <- left_join(prot_dt,samples,by="Sample")
...
prot_dt$Module <- partition[prot_dt$Accession]

# Cast data into a dm, summarize all proteins in a module.
dm <- prot_dt %>% group_by(Module, Model, SampleType) %>% 
	dplyr::summarize(Sum.Intensity=sum(Intensity),.groups="drop") %>% 
	as.data.table() %>%
	dcast(Module ~ Model + SampleType,value.var="Sum.Intensity") %>%
...
dge$samples$background <- as.factor(sapply(strsplit(groups,"_"),"[",1))
dge$samples$genotype <- as.factor(sapply(strsplit(groups,"_"),"[",2))

treatment <- sapply(strsplit(groups,"_"),"[",2)
treatment[which(treatment == "HET")] <- "KO"
dge$samples$treatment <- treatment

# Create design matrix.
design <- model.matrix( ~ background + treatment, data=dge$samples)
...
data(cortex_partition)
partition <- cortex_partition
names(partition) <- gene_map$uniprot[match(names(partition),gene_map$ids)]

data_in <- impute_protein

data(cortex_data)
#data_in <- cortex_data$Data
#idx <- match(colnames(data_in), sample_info$SampleID)
#colnames(data_in) <- sample_info$ColumnName[idx]
#data_in <- 2^data_in

prot_data <- data_in %>% 
	reshape2::melt(id.var=c("Accession","Peptides"),
		       variable.name="Sample",value.name="Intensity")
#colnames(prot_data) <- c("Accession","Sample","Intensity")

idx <- match(prot_data$Sample,sample_info$ColumnName)
prot_data$Background <- sample_info$Model[idx]
prot_data$Genotype <- sample_info$SampleType[idx]
prot_data$Module <- partition[prot_data$Accession]
prot_data <- prot_data %>% filter(Module != 0, Genotype != "QC")
prot_data$Treatment <- gsub("KO|HET","Mutant",prot_data$Genotype)
prot_data$Treatment <- gsub("WT","Control",prot_data$Genotype)

dm <- prot_data %>% group_by(Module,Background,Genotype) %>% 
	dplyr::summarize(Sum.Intensity = sum(Intensity), .groups="drop") %>%
	as.data.table() %>%
	dcast(Module ~ Background + Genotype, value.var = "Sum.Intensity") %>%
	as.matrix(rownames=TRUE)
rownames(dm) <- paste0("M",rownames(dm))


dge <- DGEList(counts=dm)
dge <- calcNormFactors(dge)
dge$samples$background <- sapply(strsplit(rownames(dge$samples),"_"),"[",1)
dge$samples$genotype <- sapply(strsplit(rownames(dge$samples),"_"),"[",2)

dge$samples$treatment <- "Mutant"
dge$samples$treatment[dge$samples$genotype == "WT"] <- "Control"
...
modules <- split(names(cortex_partition),cortex_partition)
modules["74"]
