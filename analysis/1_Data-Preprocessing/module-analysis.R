...
# Load the network.
adjm <-cortex_data[["Adjm"]]
ne_adjm <-cortex_data[["Netw"]]
...
prot_dt <- cortex_data$Data %>% as.data.table(keep.rownames="Accession") %>%
	reshape2::melt(id.var="Accession", value.var="Intensity")
colnames(prot_dt) <- c("Accession","Sample","Intensity")
prot_dt$Intensity <- 2^prot_dt$Intensity # un-log
