...
## Otherwise, parse command line argument as analysis_type:
} else if (!interactive()) {
...
final_protein <- left_join(filt_protein,samples,by=cols) %>% 
	as.data.table()
...
myfile <- file.path(rdatdir,paste0(output_name,"_norm_protein.csv"))
