...
# Use ctl+shift+c to comment selected text!
...
# Creat otuput directory for figures and table. 
...
file <- paste0(Rdatadir, "/", "Combined_TMT_Analysis_TAMPOR_GLM_Results.RDS")
...
connectivity <- lapply(subdat, function(x) softConnectivity(
  datExpr = t(x),
  corFnc = "bicor",
  weights = NULL,
  type = "signed",
...
# Examine fit.
fit <- lapply(connectivity, function(x) ggplotScaleFreePlot(x))
fit$wt$ggplot
fit$ko$ggplot
...
sampled_params$score <- sampled_params$q2/(sampled_params$PercentGrayNodes/100)
...
# file <- paste0(outputfigs, "/", outputMatName, "Network_Parameters.tiff")
...
# Input for NetRep:
r <- bicor(t(subdat$wt)) # Data has already be log-transformed.
adjm <- ((1 + r) / 2)^power_beta$wt # Signed network.

data_list <- list(data = t(subdat$wt)) # The protein expression data.
...
module_labels <- net$wt$colors # Module labels.
names(module_labels) <- rownames(subdat$wt)
...
#-------------------------------------------------------------------------------
#' ## Determine module preservation between WT and KO networks. 
#-------------------------------------------------------------------------------

library(NetRep)

# Calculate signed adjacency matrices. 
r <- lapply(subdat, function(x) bicor(t(x)))
f <- function(r,power){
  adjm <- ((1+r)/2)^power
  return(adjm)
}
adjm <- mapply(f,r,power_beta, SIMPLIFY = FALSE)

# Netrep input:
data_list <- list(wt = t(subdat$wt), # Insure you have transformed the data!
                  ko = t(subdat$ko)) # The protein expression data. 
correlation_list <- r                # The bicor correlation matrix.
network_list <- adjm                 # The weighted, signed co-expresion network.
...
preservation <- NetRep::modulePreservation(
  ...
  nThreads = 8, # Increase nPerm to 100,000 in order to stabilize the result with large number of modules.
  null = "overlap",
  alternative = "greater",
  simplify = TRUE,
  verbose = TRUE)
...
#-------------------------------------------------------------------------------
#' ## Save/Load the workspace to avoid re-running modulePreservation()
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#' ## Examine ME network of convergent modules.
#-------------------------------------------------------------------------------
...
# Create igraph.
subg <- graph_from_adjacency_matrix(sub, mode = "undirected", weighted = TRUE)
