...
for (i in 1:length(godata)){
  color <- colorvec[i]
  plots[[i]] <- ggplotGOscatter(godata, color = color)
}
...
#-------------------------------------------------------------------------------
#' ## Evaluate go term similarity between DEP communities.
#-------------------------------------------------------------------------------

# Entrez genes for each DEP community (cluster).
clusters <- sapply(community_results,"[",4)
names(clusters) <- names(community_results)
