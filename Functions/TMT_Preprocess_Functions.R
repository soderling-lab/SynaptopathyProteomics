...
plotExp_MDS <- function(data_in,exp){
  ...
  p %<a-% plotMDS(log2(dm), col = c(rep('gray',3),rep('blue',4),rep('red',4)), 
                  main = title)
  p
}
...
df2dm_TMT <- function(df,colID){
  ...
  rownames(dm) <- if("Accession" %in% df) { df$Accession } else { rownames(df) }  #***EBD edited
...
filterQC <- function(data_in,groups,nbins,threshold){
  for (i in 1:length(groups)){
    ...
    data_sub$Intensity_bin[!rows_ignore] <- ntile(data_sub$QC_Average[!rows_ignore], nbins, na.rm = TRUE, 
                                                  checkBleed = FALSE, result = "numeric") 
    
    # Calculate summary statistics of Intensity bins
    sd_info <- subset(data_sub) %>% group_by(Intensity_bin) %>% 
      summarize(min = min(QC_Average,na.rm=TRUE), 
...
summarize_Protein <- function(peptide_data) {
  ...
  prot_data <- temp_data %>% group_by(Accession) %>% summarise_all(funs(sum), na.rm = TRUE)
  #Replace 0 with NA
...
checkNormalization <- function(data_in,traits,colors,title){
  # Generate plots. 
  p1 <- ggplotBoxPlotv2(log2(data_in),colID="b",traits=traits,
                        colors=colors,
                        title=title)
  p2 <- ggplotDensityv2(log2(data_in),colID="b",traits=traits,
                        colors=colors,
                        title=title)
  p3 <- ggplotMDS(log2(data_in),colID="b",traits, 
                  title=title) + theme(legend.position = "none")
...
ggsaveMultiPDF <- function(plot_list,savefigs_path,file_ext,verbose=TRUE){
  ...
  # Coerce to list.
  if (!inherits(plot_list,"list")){plot_list <- list(plot_list)}
  # Loop to save plots.
...
ggplotQCHist <- function(data_in, group, nbins, threshold){
  ...
  for (j in 1:nbins) {
    ...
    g <- gtable_add_grob(tab, grobs = rectGrob(gp = gpar(fill = NA, lwd = 1)),
                         t = 1, b = nrow(tab), l = 1, r = ncol(tab))
    plot <- plot + annotation_custom(g, xmin = xmin+0.80*xdelta, xmax, 
...
eBLM_exactTest <- function(data_in, geno, sample_info, cov, OLS, col){
  ...
  Entrez <- mapIds(org.Mm.eg.db, keys = Uniprot_IDs, 
  ...
  names(results_list) <- c("fit.eblm","y_DGE","y_TT","GO","KEGG","nsig",
...
filter_proteins <- function(data_in,colID){
  ...
  print(paste(length(out[out==TRUE]),
...
edgeR_GSE <- function(qlf, FDR=0.05, filter=TRUE){
  GSE_results <- list()
  # Map Uniprot IDs to Entrez IDs:
  Entrez <- mapIds(org.Mm.eg.db, keys = rownames(qlf), 
                   column = "ENTREZID", 
                   keytype = "UNIPROT", 
                   multiVals = "first")
...
ggplotVerboseBoxplot <- function(x,g,levels,contrasts,color,stats=FALSE, 
  ...
  if (method == "dunn"){
    ...
    dupDtest <- Dtest
    dupDtest$Comparison <- do.call(rbind,
                                   lapply(strsplit(Dtest$Comparison," - "), 
...
ggplotFreqOverlap <- function(data_in,colID,groups){
  ...
  plot <- ggplot(df, aes(x = Var1, y = Freq, fill = Var1)) +
    ...
    labs(title = "Identification Overlap",
  ...
  plot <- plot + annotate("text", x = df$Var1 , y = df$ypos, 
...
ggplotSigOverlap <- function(data,protID,colID){
  ...
  plot <- ggplot(df, aes(x = Var1, y = Freq, fill = Var1)) +
    geom_col() +
    scale_fill_grey(start = 0.8, end = 0.2) +
    labs(title = "Differentially Expressed Protein Overlap",
         x = "Experiment Overlap",
         y = "Protein Frequency") +
    theme(
      plot.title = element_text(hjust = 0.5, color="black", size=14, face="bold"),
...
mixcolors <- function(color1,color2,ratio1=1,ratio2=1,plot=TRUE){
...
ggplotProteinScatterPlot <- function(exprDat,protein1,protein2){
  
  # Get data for proteins of interest.
  x <- as.numeric(exprDat[rownames(exprDat)==protein1,])
  y <- as.numeric(exprDat[rownames(exprDat)==protein2,])
  
  # Bind as df for ggplot.
  df <- as.data.frame(cbind(prot1=x,prot2=y))
  
  # Determine best fit line. 
  fit <- lm(df$prot1 ~ df$prot2)
  ...
  plot <- ggplot(df, aes(x = prot1, y = prot2)) + 
    ...
    geom_abline(intercept=coef(fit)[1],slope=coef(fit)[2], color = "black", linetype = "dashed") + 
    ggtitle(NULL) + 
    xlab(paste("Log2(Expression ",protein1,")",sep="")) + 
    ylab(paste("Log2(Expression ",protein2,")",sep="")) +  
    theme(
      plot.title = element_text(hjust = 0.5, color="black", size=11, face="bold"),
      axis.title.x = element_text(color="black", size=11, face="bold"),
      axis.title.y = element_text(color="black", size=11, face="bold"))
...
annotate_sig <- function(plot,stats,group,annotate=TRUE){
  ...
  label.df <- as.data.frame(cbind(rownames(stats),
                                  stats$FDR,group,
                                  stats$logFC))[idx,]
  ...
  # Add ypos.
  groupMax <- subset(plot$data) %>% group_by(Group) %>% dplyr::summarize(max=max(Intensity))
...
makePairwiseContrasts <- function(g1, g2, collapse = " - "){
  ...
  if (!inherits(g1,"list")){g1 <- as.list(g1)}
  if (!inherits(g2,"list")){g2 <- as.list(g2)}
